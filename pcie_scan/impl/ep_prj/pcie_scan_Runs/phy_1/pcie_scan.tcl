...
set rc [catch {
  open_project {pcie_scan.prj}
  import_device ph1_400.db -package PH1A400SFG900 -speed 2
  import_db {../syn_1/pcie_scan_gate.db}
  read_sdc ../../constraints/case_ep.sdc
  place
  export_db {pcie_scan_place.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  route
  report_area -io_info -file pcie_scan_phy.area
  export_db {pcie_scan_pr.db}
  start_timer
  report_timing -mode FINAL -net_info  -rpt_autogen true -file pcie_scan_phy.timing
} RESULT]
if {$rc} {
...
set rc [catch {
  export_bid pcie_scan_inst.bid
  bitgen -bit "pcie_scan.bit"
} RESULT]
if {$rc} {
  step_error bitgen
  return -code error $RESULT
