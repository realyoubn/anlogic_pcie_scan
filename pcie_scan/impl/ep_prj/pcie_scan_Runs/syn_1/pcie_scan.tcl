...
set rc [catch {
  ...
  elaborate -top {pcie_scan}
  export_db {pcie_scan_elaborate.db}
} RESULT]
if {$rc} {
...
set rc [catch {
  read_adc ../../constraints/case_ep.adc
  optimize_rtl
  report_area -file pcie_scan_rtl.area
  export_db {pcie_scan_rtl.db}
} RESULT]
if {$rc} {
  step_error opt_rtl
  return -code error $RESULT
...
set rc [catch {
  ...
  optimize_gate -maparea pcie_scan_gate.area
  legalize_phy_inst
  export_db {pcie_scan_gate.db}
} RESULT]
if {$rc} {
  step_error opt_gate
  return -code error $RESULT
